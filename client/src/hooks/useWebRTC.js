...
export const useWebRTC = (roomId, user, owner) => {
	...
	useEffect(() => {
		...
		const handleNewPeer = async ({ peerId, createOffer, user: remoteUser }) => {
				...
				addNewClient({ ...remoteUser, muted: true }, () => {
					...
					if (remoteUser.id === "audioplayer") {
						if (audioElements.current["audio-player"]) {
							audioElements.current["audio-player"].srcObject = e.streams[1];

							console.log("remoteStream 1", e.streams[1]);
						} else {
							let settled = false;
							const interval = setInterval(() => {
								if (audioElements.current[remoteUser.id]) {
									audioElements.current[remoteUser.id].srcObject = e.streams[1];

									console.log("remoteStream 2", e.streams[1]);
									settled = true;
								}
						...
						if (audioElements.current[remoteUser.id]) {
							audioElements.current[remoteUser.id].srcObject = e.streams[0];
			...
			if (audioStream.current)
				console.log("Audio stream tracks:", audioStream.current);

			if (owner) {
				const audioTracks = audioStream.current.getAudioTracks();
				const localTracks = localMediaStream.current.getTracks(); // This can include audio/video tracks
				localMediaStream.current.getAudioTracks().forEach((track) => {
					track.enabled = true; // Ensure track is unmuted
				});
				...
				localTracks.forEach((track) => {
					// Ensure the track isn't already added from audioStream
					if (
